...
import { UtensilsCrossed, DollarSign, Package, Upload, AlertCircle, Save, ArrowLeft } from "lucide-react"

// Main Component
export default function MenuItemEditForm() {
  const router = useRouter()
  const params = useParams()
  const itemId = params.id // Get the ID from the URL
  ...
  // Form state for menu item - with default values for editing
  const [menuItem, setMenuItem] = useState({
    itemCode: `FOOD00${itemId || '1'}`,
    name: itemId === '1' ? "Classic Burger" : itemId === '2' ? "Margherita Pizza" : "Menu Item",
    category: itemId === '1' ? "Main Course" : itemId === '2' ? "Pizza" : "Main Course",
    price: itemId === '1' ? "12.99" : itemId === '2' ? "18.50" : "10.99",
    description: itemId === '1' ? "Juicy beef patty with fresh lettuce and tomato" : itemId === '2' ? "Classic pizza with fresh mozzarella and basil" : "Delicious menu item",
    ingredients: itemId === '1' ? "Beef patty, lettuce, tomato, onion, pickles, cheese, bun" : itemId === '2' ? "Pizza dough, tomato sauce, mozzarella cheese, fresh basil" : "Various ingredients",
    allergies: itemId === '1' ? "Contains gluten, dairy" : itemId === '2' ? "Contains gluten, dairy" : "Please check ingredients",
    stock: itemId === '1' ? "25" : itemId === '2' ? "15" : "20",
    status: itemId === '1' ? "Available" : itemId === '2' ? "Available" : "Available",
    preparationTime: itemId === '1' ? "15" : itemId === '2' ? "20" : "10",
    calories: itemId === '1' ? "520" : itemId === '2' ? "680" : "400",
    isVegetarian: itemId === '2' ? true : false,
    isVegan: false,
    isGlutenFree: false,
  })
  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(formErrors).length === 0) {
      ...
      try {
        ...
        console.log("Updated menu item data:", updatedMenuItemData)
        ...
        // Navigate back to menu list after successful update
  ...
  return (
    <div className="flex min-h-screen bg-white overflow-hidden">
      <Head>
        <title>Edit Menu Item - Fudo Admin</title>
        <meta name="description" content="Edit menu item in your restaurant" />
          ...
          <div className="mb-8">
            <div className="flex flex-col items-start gap-6 text-[#333333]">
              <div className="text-sm flex items-center font-poppins">
                ...
                <span className="mx-2 text-lg">›</span>
                <span>Edit Item #{itemId}</span>
              </div>
              <div className="flex items-center justify-between w-full">
                <h1 className="text-2xl lg:text-3xl font-bold font-poppins">Edit Menu Item #{itemId}</h1>
              </div>
            </div>
          </div>
